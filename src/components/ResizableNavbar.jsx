...
const Navbar = ({ children, className = '' }) => {
  ...
  useMotionValueEvent(scrollY, "change", (latest) => {
    ...
    // Control hide/show on scroll, now triggered after scrolling past the viewport height
    ...
    <>
      ...
      >
        ...
        <motion.div
          ...
          className={cn(
            "relative z-[60] mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-full bg-transparent px-4 py-2 lg:flex dark:bg-transparent transition-colors duration-300 ease-in-out",
            (visible || !isHomePage) && "bg-white/80 dark:bg-neutral-950/80"
          )}
        >
          {/* Logo */}
        ...
        <motion.div
          className={cn(
            "relative z-50 mx-auto flex w-full max-w-[calc(100vw-2rem)] flex-col items-center justify-between bg-transparent px-0 py-2 lg:hidden transition-colors duration-300 ease-in-out",
            (visible || !isHomePage) && "bg-white/80 dark:bg-neutral-950/80"
          )}
          animate={{
            backdropFilter: visible ? "blur(10px)" : "none",
